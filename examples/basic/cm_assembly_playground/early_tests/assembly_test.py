...
finger_urdf ="abondance_finger.urdf"

finger0 = p0.loadURDF(finger_urdf)
finger1 = p1.loadURDF(finger_urdf)
...
startPos = [0, 0, 0]  # have to be careful to set this position such that the box and atuator just touch (to replicate experimental condition)
startOr = p.getQuaternionFromEuler([0, 0, 0])
assemblyId = p.loadURDF(
    "combined.urdf", basePosition=startPos, baseOrientation=startOr, physicsClientId=physicsClient
)
