...
class CondenseData:
    ...
    def set_label_columns(
        self, label, cols, tie_to_input, input_vals, scales=None, col_labels=None
    ):
    ...
    def _calculate_condense(self, label, filter_cols=None):
        ...
        if self.label_columns.get(label, None) is not None:
            ...
            for input_val in label_settings["cols"]:
                ...
                if len(Z_label) == 0:
                    ...
                    out["percentile25"].append(
                        percentile25 * label_settings["scales"][input_val]
                    )
                    out["percentile50"].append(
                        percentile50 * label_settings["scales"][input_val]
                    )
                    out["percentile75"].append(
                        percentile75 * label_settings["scales"][input_val]
                    )
        ...
        if self.label_columns.get(label, None) is not None:
            ...
            for input_val in label_settings["cols"]:
                ...
                if len(Z_label) == 0:
                    ...
                    out["max"].append(
                        z_curr[max_idx] * label_settings["scales"][input_val]
                    )
                    out["min"].append(
                        z_curr[min_idx] * label_settings["scales"][input_val]
                    )
    ...
    def count_true(self, column, value=1.0):
        counts = self.iterate_over_function(
            self._count_true, column=column, value=value
        )
    ...
    def plot(self, x, y, labels, suffix="", **kwargs):
        ...
        if isinstance(self.overall[y], dict):
            ...
            for y_entry, y_error, label in zip(y_vals, y_err, labels):
                ...
                plt.fill_between(
                    df[x].values, y_entry - y_error, y_entry + y_error, alpha=0.3
                )
        else:
    ...
    def contour_plot(self, x, y, z, labels, suffix="", subfield=None, **kwargs):
        ...
        if isinstance(self.overall[z], dict):
            ...
            length = len(labels)
            for z_entry, label, idx in zip(z_vals, labels, range(len(labels))):
                mask = np.isnan(z_entry)
                z_entry = np.array(z_entry)[~mask]
        ...
        figname = os.path.join(
            base_folder,
            str(z) + "_vs_" + str(x) + "_and_" + str(y) + suffix + "_contour.png",
        )
        plt.savefig(figname, dpi=450)
        plt.savefig(figname.replace(".png", ".svg"), dpi=450)
    ...
    def plot_percentiles(self, x, y, labels, suffix="", **kwargs):
        ...
        if isinstance(self.overall[y], dict):
            ...
            for y_entry, y_error_p, y_error_n, label in zip(
                y_vals, y_err_pos, y_err_neg, labels
            ):
                y_entry = np.array(y_entry)
                ...
                plt.plot(df[x].values, y_entry, label=label, **kwargs)
                plt.fill_between(
                    df[x].values, y_entry - y_error_n, y_entry + y_error_p, alpha=0.3
                )
        else:
    ...
    def condense(self, labels=None, inputs=None, filter_cols=None):

        for label in labels:
            dat = self.iterate_over_function(
                self._calculate_condense, label=label, filter_cols=filter_cols
            )
