...
class IHMPerformance:
    ...
    def set_label_columns(self,cols, tie_to_input, input_vals, scales=None, labels=None):
        if labels is None:
            labels=cols
    ...
    def _plot_label(self, results, label_name, success_filename, inputs=None, iterate=None, filter_col=None, show=False, replace=False, aux_savepath=None):
        ...
        for label in label_name:
            ...
            for curr_iter_val in iter_vals:
                ...
                plt.savefig(outfile+'_'+label+'_'+str(curr_iter_val)+'.png', dpi=450)
                plt.savefig(outfile+'_'+label+'_'+str(curr_iter_val)+'.svg', dpi=450)

                # Save the plot in the second location if one is given
                if aux_savepath is not None:
                    aux_path = aux_savepath.replace('.yaml', '')
                    plt.savefig(aux_path+'_'+label+'_'+str(curr_iter_val)+'.png', dpi=450)
    ...
    def plot_one(self, success_filename, inputs = None, labels=None, iterate=None, filter_col=None, show=False, replace=False, aux_savepath=None):
        ...
        if len(results['vars']) ==2:
            ...
            for key in label_data:
                self._plot_label(results, key, success_filename, inputs=inputs, iterate=iterate, filter_col=filter_col, show=show, replace=replace, aux_savepath=aux_savepath)
        else:
            raise ValueError('You are trying to plot a 2D grid with %dD data. You can only plot grids of 2D data'%(len(results['vars'])))


    def plot_multi(self, success_filename, label_groups=[],labels=None, show=False, replace=False, aux_savepath=None):
        """Make a plot of the grasp type/success rate of 2D sweep data."""
        ...
        try:
            ...
            for keys in label_groups:
                self._plot_label(results, keys, success_filename, show=show, replace=replace, aux_savepath=aux_savepath)
        


    def make_plots(self, labels=None, inputs=None, iterate=None, filter_col=None, show=False, recalculate=False, num_bins = 12):
        ...
        for folder in folders:
            ...
            self.plot_one(success_filename, inputs=inputs, labels=labels, iterate=iterate, filter_col=filter_col, show=show, replace=recalculate, aux_savepath=aux_savepath)