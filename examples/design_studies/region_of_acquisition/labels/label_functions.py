...
def final_pose(objectpose, **kwargs):
    ...
    """
    last_pose = objectpose.iloc[-1][['posX', 'posY','posZ','eulerX','eulerY','eulerZ']]
...
def is_grasp(contact, **kwargs):
    ...
    contacts_prefix = contact[contact.stepCount == last_time]
...
def mean_link(contact, **kwargs):
    ...
    # Get the average contact location
    finger_conditionA = [False]*len(contacts_prefix)
    finger_conditionB = [False]*len(contacts_prefix)
    for fingerID in fingerIDs:
        fconditionA = contacts_prefix.bodyUniqueIdA == fingerID
        fconditionB = contacts_prefix.bodyUniqueIdB == fingerID
        finger_conditionA = finger_conditionA | fconditionA
        finger_conditionB = finger_conditionB | fconditionB

    object_conditionA = [False]*len(contacts_prefix)
    object_conditionB = [False]*len(contacts_prefix)
    for objectID in objectIDs:
        oconditionA = contacts_prefix.bodyUniqueIdA == objectID
        oconditionB = contacts_prefix.bodyUniqueIdB == objectID
        object_conditionA = object_conditionA | oconditionA
        object_conditionB = object_conditionB | oconditionB
    
    contacts_fingersA = contacts_prefix[finger_conditionA & object_conditionB]
    contacts_fingersB = contacts_prefix[finger_conditionB & object_conditionA]

    fcontactsA = contacts_fingersA['linkIndexA'].to_list()
    fcontactsB = contacts_fingersB['linkIndexB'].to_list()

    last_contact_links = []
    last_contact_links.extend(fcontactsA)
    last_contact_links.extend(fcontactsB)


    if len(last_contact_links)>0:
        mean_link = np.mean(last_contact_links) + 1
    else:
        mean_link = None
...
def mean_link_4finger(objectpose, contact=None):
    ...
    if contact is not None:
        ...
        last_contacts = last_contacts[((last_contacts.bodyUniqueIdA == 1) | \
                                       (last_contacts.bodyUniqueIdA == 2) | \
                                       (last_contacts.bodyUniqueIdA == 3) | \
                                       (last_contacts.bodyUniqueIdA == 4)   ) & (last_contacts.bodyUniqueIdB == 5)]
    ...
    if posZ[-1]>0.1:
        return int(mean_link) # The success label is the average contact location wit the object.
    # Otherwise, fail
    else:
        return 0
...
def grasp_types(objectpose, contact=None):
    ...
    if contact is not None:
        ...
        last_time = contact['stepCount'].iloc[-1]
        last_contacts = contact[contact.stepCount == last_time]
        last_contacts = last_contacts[((last_contacts.bodyUniqueIdA == 1) | (last_contacts.bodyUniqueIdA == 2)) & (last_contacts.bodyUniqueIdB == 3.0)]
